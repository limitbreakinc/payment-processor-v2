...
contract ModuleBulkBuyListingsForSelfTest is cPortModuleTest {

    function testBulkBuyListingForSelf_ETH(FuzzedOrder721[2] memory fuzzedOrderInputsArray) public {
    ...
    function testBulkBuyListingForSelf_WETH(FuzzedOrder721[2] memory fuzzedOrderInputsArray) public {
        for (uint256 i = 0; i < fuzzedOrderInputsArray.length; ++i) {
            _scrubFuzzedOrderInputs(fuzzedOrderInputsArray[i]);
        }
    ...
    function _bulkBuyListingsForSelf(address purchaser, uint128 nativePaymentValue, FuzzedOrder721[] memory fuzzedOrderInputsArray, Order[] memory saleDetailsArray, bytes4 expectedRevertSelector) private {
        ...
        _cPort.bulkBuyListingsForSelf{value: nativePaymentValue}(fnCalldata);
    }
}