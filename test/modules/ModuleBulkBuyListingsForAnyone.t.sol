...
contract ModuleBulkBuyListingsForAnyoneTest is cPortModuleTest {

    function testBulkBuyListingForAnyone_ETH(FuzzedOrder721[2] memory fuzzedOrderInputsArray) public {
    ...
    function testBulkBuyListingForAnyone_WETH(FuzzedOrder721[2] memory fuzzedOrderInputsArray) public {
        for (uint256 i = 0; i < fuzzedOrderInputsArray.length; ++i) {
            _scrubFuzzedOrderInputs(fuzzedOrderInputsArray[i]);
        }
    ...
    function _bulkBuyListingsForAnyone(address purchaser, uint128 nativePaymentValue, FuzzedOrder721[] memory fuzzedOrderInputsArray, Order[] memory saleDetailsArray, bytes4 expectedRevertSelector) private {
        ...
        _cPort.bulkBuyListingsForAnyone{value: nativePaymentValue}(fnCalldata);
    }
}